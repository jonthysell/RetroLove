...

{
    ...
    Write-Host "Building breakout.love..."
    Remove-Item -Recurse -Force "$scriptDir\build" | Out-Null
    New-Item "$scriptDir\build" -Type directory -Force | Out-Null
    Compress-Archive -Path "$scriptDir\*.lua","$scriptDir\*.ogg" -DestinationPath "$scriptDir\build\breakout.zip" -CompressionLevel "Optimal" -Force | Out-Null
    Rename-Item -Path "$scriptDir\build\breakout.zip" -NewName "breakout.love" -Force | Out-Null
